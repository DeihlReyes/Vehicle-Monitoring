...
async def ws():
    ...
    try:
        # Send the full cumulative session data immediately on connect
        try:
            session_id = get_or_create_singleton_session()
            session_data = db_manager.get_session_data(session_id)
            # Add a type field for frontend distinction
            await client.send(json.dumps({"type": "init", **session_data}))
        except Exception as e:
            logger.error(f"Failed to send initial session data to new WebSocket client: {str(e)}")
